...
def augment_hsv(im, hgain=0.5, sgain=0.5, vgain=0.5):
    """
    Args:  HSV color-space augmentation HSV 色调变换
    通过在ＨＳＶ色彩空间中，对h、s、v三个通道增加扰动，来进行色调变换，达到数据增强的目的
        im: 传入的图片
        hgain:　h的扰动系数
        sgain:　s的扰动系数
        vgain:　v的扰动系数

    Returns:　　返回色调扰动后的BGR图像
    """

    if hgain or sgain or vgain:
        # 随机抽取-1 ~ 1 之间的三个实数，乘以hsv三通道扰动系数
        r = np.random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
        hue, sat, val = cv2.split(cv2.cvtColor(im, cv2.COLOR_BGR2HSV)) # 图像的通道拆分
        ...
        #　使用cv2.LUT()函数　将通道中的值进行构建，生成新的通道　然后合并通道
        im_hsv = cv2.merge(
