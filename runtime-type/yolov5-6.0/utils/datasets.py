...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations　库　数据增强工具  这里面进行的是传统图像变化
            img, labels = self.albumentations(img, labels)
            nl = len(labels)  # update after albumentations

            # HSV color-space  像素上的变化
...
def load_image(self, i):
    ...
    if im is None:  # not cached in ram
        ...
        if npy and npy.exists():  # load npy
            ...
            path = self.img_files[i] # 读取突破路径
            im = cv2.imread(path)  # BGR　
            assert im is not None, "Image Not Found " + path
        h0, w0 = im.shape[:2]  # orig hw
        r = self.img_size / max(h0, w0)  # ratio　　＃ 确定缩放比例
...
def load_mosaic(self, index):
    """
    将多张图片进行拼接成一张图片，这里一般使用４张
    Args:
        self:
        index:需要用到的图像索引
    Returns:　　img4: mosaic和随机透视变换后的一张图片  numpy(640, 640, 3)
             labels4: img4对应的target  [M, cls+x1y1x2y2]
    """
    # YOLOv5 4-mosaic loader. Loads 1 image + 3 random images into a 4-image mosaic
    # labels4 用于存放拼接图像的标签信息（不包含segments多边形)
    # segments4 用于存放拼接图像的标签信息（包含segments多边形,实例对象的分割点）
    labels4, segments4 = [], []
    s = self.img_size # 图像大小
    ...
    for j, (i, index) in enumerate(zip(ri, indices)):
        ...
        if i == 0:  # top left
            ...
            # w=736  h = 552  马赛克图像：(x1a,y1a)左上角 (x2a,y2a)右下角
            x1a, y1a, x2a, y2a = (
                max(xc - w, 0),
                max(yc - h, 0),
                xc,
        ...
        # 将图像img的【(x1b,y1b)左上角 (x2b,y2b)右下角】
        # 区域截取出来填充到马赛克图像的【(x1a,y1a)左上角 (x2a,y2a)右下角】区域
    ...
    # Concat/clip labels   标签拼接到一起
    labels4 = np.concatenate(labels4, 0)
    # 防止越界  label[:, 1:]中的所有元素的值（位置信息）必须在[0, 2*s]之间,
    # 小于0就令其等于0,大于2*s就等于2*s   out: 返回
